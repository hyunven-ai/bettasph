  ...
  return (
    <Link href={`/ikan/${product.slug}`} className="group block h-full">
      <div className="h-full bg-[#0B1120] rounded-[2.5rem] overflow-hidden border border-white/5 hover:border-[var(--color-brand-aqua)]/40 transition-all duration-500 ease-senior flex flex-col relative before:absolute before:inset-0 before:bg-gradient-to-b before:from-transparent before:via-transparent before:to-black/90 before:z-10 group-hover:shadow-[0_20px_40px_rgba(0,0,0,0.4)]">
        ...
        {/* Glow on hover */}
        <div className="absolute top-1/4 left-1/2 -translate-x-1/2 w-40 h-40 bg-[var(--color-brand-aqua)] rounded-full blur-[80px] opacity-0 group-hover:opacity-20 transition-opacity duration-1000 pointer-events-none" />

        {/* Discount badge */}
        {hasDiscount && (
          <div className="absolute top-6 right-6 z-30 px-2.5 py-1 bg-rose-500 text-white text-[11px] font-black rounded-full shadow-lg">
            -{discount}%
          </div>
        )}
        ...
        <div className="relative aspect-[3/4] w-full overflow-hidden bg-[#050810]">
          ...
          {/* Arrow icon — only show when no discount badge overlaps */}
          {!hasDiscount && (
        ...
        <div className="p-8 relative z-20 flex-1 flex flex-col justify-end mt-auto -mt-24 pointer-events-none">
          <div className="mb-4">
            <h3 className="font-outfit text-2xl font-bold text-white group-hover:text-[var(--color-brand-aqua)] transition-colors leading-tight tracking-tight">
              {product.name}
            </h3>
            <p className="text-slate-400 text-xs mt-1 font-medium tracking-wide">
              Size {product.size} — {product.grade || "Premium Grade"}
            </p>
          </div>
          ...
          <div className="flex justify-between items-center bg-white/5 backdrop-blur-lg border border-white/10 rounded-2xl p-4 pointer-events-auto">
            <div className="flex flex-col">
              <span className="text-[9px] text-slate-400 uppercase tracking-[0.15em] font-bold mb-1 opacity-60">
                Mulai dari
              </span>
              ...
              {hasDiscount ? (
                <div className="flex flex-col gap-0.5">
                  <span className="font-outfit font-black text-white text-xl tracking-tight leading-none">
                    {formatPrice(finalPrice)}
                  </span>
                  <span className="text-slate-500 text-[11px] line-through font-medium leading-none">
